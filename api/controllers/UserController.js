...
module.exports = {
  ...
  register: function (req, res) {
    ...
    if (pass.length < 8) { return res.json(400, {
  ...
  updateData: function (req, res) {
    ...
    var mail = req.param('email')
    if (!u.verifyPassword(old_digest)) { return res.json(401, {
  ...
  login: function (req, res) {
    User.findOne({
      ...
      if (!user.verifyPassword(req.param('digest'))) { return res.json(401, {
        message: 'User not found.'
      }) }
  ...
  resetpassword: function (req, res) {
    if (!req.param('tempToken')) { return res.json({
